...
<Aside>
  대부분의 단일 사용자 인스턴스에서는 기본값인 `all` 모드로 충분합니다.
  워커 분리는 주로 수천 명의 팔로워가 있거나 연합 활동이 많은 인스턴스에 유용합니다.
  
  `web` 모드를 사용할 때는 들어오는 액티비티를 처리하고 나가는 포스트를 전송하기 위해
  최소 하나의 `worker` 노드가 실행 중이어야 합니다.
</Aside>
...
액티비티 처리 중(예: 부스트나 새 게시물 수신 시) 오래된 액터를 만나면
해당 프로필 데이터가 비동기적으로 새로고침됩니다.

기본값은 `7`입니다.

### `REFRESH_ACTORS_ON_INTERACTION` <Badge text="선택" />

`true`로 설정하면 모든 수신 액티비티(좋아요, 에모지 리액션, 팔로우 등)에서
...
이 로그는 구조화 로그 형식(기본값: JSON Lines)으로 기록됩니다.

### `LOG_FILE_FORMAT` <Badge text="선택" />

[`LOG_FILE`](#log_file)로 지정한 로그 파일의 형식.  다음 값 중 하나를 선택하세요:

 -  `jsonl` (기본값): [JSON Lines] 형식.  한 줄에 JSON 객체 하나씩 출력합니다.
    구조화된 JSON을 파싱하는 로그 집계 도구에 적합합니다.
 -  `logfmt`: [logfmt] 형식.  한 줄에 키-값 쌍으로 출력합니다.
    사람이 읽기 쉽고 [Loki] 등의 도구와 호환됩니다.
...
### `FEDIFY_DEBUG` <Badge text="선택" />

`true`로 설정하면 [Fedify 디버거]를 활성화합니다. Fedify 디버거는 ActivityPub
...
<Aside type="caution">
  Fedify 디버거는 개발 목적으로만 사용하도록 설계되어 있습니다. 프로덕션 환경에서
  이 옵션을 활성화하면 내부 트레이스 데이터가 외부에 노출되어 보안상 위험합니다.
</Aside>

[Fedify 디버거]: https://fedify.dev/manual/debug
